...
final class ModeSettingCard extends SettingCardComponent<ModeSetting> {
    ...
    float height(SettingCardContext context) {
        ...
        return 33.0F + descriptionOffset(context, 124.0F) + setting.getModes().size() * 15.0F * context.clamp(state.expandAnimation.getOutput().floatValue(), 0.0F, 1.0F);
    }

    @Override
    float layoutHeight(SettingCardContext context) {
        return 33.0F + descriptionOffset(context, 124.0F);
    }
    ...
    void render(GuiGraphics graphics, SettingCardContext context, float cardX, float y, int mouseX, int mouseY) {
        ...
        state.expandAnimation.setDirection(state.expanded ? Direction.FORWARDS : Direction.BACKWARDS);
        List<String> descriptionLines = descriptionLines(context, 124.0F);
        ...
        if (!descriptionLines.isEmpty()) {
            context.renderWrappedText(graphics, FontType.SEMIBOLD, descriptionLines, cardX + 10.0F, y + 8.5F, 124.0F, 5F, 5.5F, new Color(145, 145, 145, 190).getRGB());
        }
