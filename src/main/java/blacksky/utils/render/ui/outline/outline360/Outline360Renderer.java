package blacksky.utils.render.ui.outline.outline360;
    ...
    private void submit(GuiGraphics graphics, BuiltOutline360 outline) {
        ...
        try {
            ...
            ((GuiGraphicsExtractorAccessor) graphics)
                    ...
                    .submitGuiElement(new Outline360RenderState(pose, normalized));
        } catch (RuntimeException ignored) {
        }
    }
    ...
    private void ensureBuffers() {
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            paramsBuffer = RenderSystem.getDevice().createBuffer(
                    () -> "BLACKSKY_outline_360_params",
                    GpuBuffer.USAGE_UNIFORM,
                    paramsData
            );
            rangesBuffer = RenderSystem.getDevice().createBuffer(
                    () -> "BLACKSKY_outline_360_ranges",
                    GpuBuffer.USAGE_UNIFORM,
                    rangesData
            );
            paramsDirty = false;
